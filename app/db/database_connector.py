...
class DatabaseConnector:
    ...
    def search_fulltext(self, table_name: str, search_columns: List[str], query_text: str, top_n: int) -> List[int]:
        ...
        if len(query_text.strip()) <= 3:
            logger.info(f"Short query detected. Using Boolean mode with wildcard for query: {query_text}")
            ...
            logger.info(f"Long query detected. Using Natural Language mode for query: {query_text}")
        ...
        logger.info(f"Executing full-text search query: {sql_query} with parameters: {search_query}, {top_n}")
        results = self.execute_query(sql_query, (search_query, top_n))
        
        if results and isinstance(results, list):
            logger.info(f"Search returned {len(results)} results.")
        else:
            logger.warning("Search returned no results or results are not in expected format.")
        
        return [row['id'] for row in results] if results else [] # type: ignore

    def search_fulltext_with_filters(self, table_name: str, search_columns: List[str], query_text: str, filters: Dict[str, Any], top_n: int) -> List[int]:
        """
        ...
        if len(query_text.strip()) <= 3:
            ...
            sql_query += " LIMIT %s"
            params = [search_query] + filter_params + [top_n]
        logger.info(f"Executing full-text search query with filters: {sql_query} with parameters: {params}")
    ...
    def _build_filter_conditions(self, filters: Dict[str, Any]) -> Tuple[str, list]:
        ...
        for column, filter_detail in filters.items():
            ...
            if not isinstance(filter_detail, dict): # Should always be a dict from FilterHandler
                logger.warning(f"Unexpected filter_detail format for column '{column}': {filter_detail}. Skipping.")
                continue
            ...
            if "values" in filter_detail:
                ...
                if value_list:
                    ...
                    logger.debug(f"  -> Built IN condition: {condition_sql}, Params added: {value_list}")
                else:
                    logger.debug(f"  -> Skipped IN condition for column '{column}' due to empty value list.")
            ...
            # Range handling (covers min_only, max_only, min_and_max)
                ...
                logger.debug(f"  -> Built RANGE condition: {column} >= {min_val} AND {column} <= {max_val}. Params added: [{min_val}, {max_val}]")
            elif "min" in filter_detail: # Only min is present
                min_val = filter_detail["min"]
                condition_sql = f"{column} >= %s"
                conditions.append(condition_sql)
                params.append(min_val)
                logger.debug(f"  -> Built MIN_ONLY range condition: {condition_sql}, Param added: {min_val}")
            elif "max" in filter_detail: # Only max is present
                max_val = filter_detail["max"]
                condition_sql = f"{column} <= %s"
                conditions.append(condition_sql)
                params.append(max_val)
                logger.debug(f"  -> Built MAX_ONLY range condition: {condition_sql}, Param added: {max_val}")
            ...
            # Exact match for a field that might otherwise be a range, or specific "exact" type
                ...
                logger.debug(f"  -> Built EXACT condition: {condition_sql}, Param added: {exact_val}")
            ...
            # For string/enum exact matches (typically filter_type "exact" or "like" from FilterConfig)
                ...
                conditions.append(condition_sql)
                params.append(val)
                logger.debug(f"  -> Built VALUE condition (assuming '='): {condition_sql}, Param added: {val}")
            else:
                # This case handles if filter_detail was an empty dict or an unrecognized structure.
                logger.warning(f"Unknown or empty filter structure for column '{column}': {filter_detail}. Skipping.")

        final_conditions_sql = " AND ".join(conditions)
        logger.debug(f"Finished building filter conditions. SQL: '{final_conditions_sql}', Params: {params}")
    ...
    def get_filtered_ids(self, table_name: str, filters: Dict[str, Any]) -> List[int]:
        ...
        logger.info(f"Executing filtered ID query: {query} with params: {filter_params}")
        ...
        if results and isinstance(results, list):
            ...
            logger.warning("Filtered ID query returned no results or results are not in expected format.")

        return [row['id'] for row in results] if results else [] # type: ignore

    def get_all_with_filters(self, table_name: str, filters: Dict[str, Any], top: int) -> List[int]:
        """
        ...
        logger.debug(f"Getting all with filters for table '{table_name}', filters: {filters}, top: {top}")

        # Reuse the consistent logic from _build_filter_conditions
        filter_sql, filter_params = self._build_filter_conditions(filters)

        query = f"SELECT id FROM `{table_name}`" # Use backticks for table name for safety
        ...
        logger.info(f"Executing get_all_with_filters query: {query} with params: {final_params}")
        results = self.execute_query(query, tuple(final_params))

        if results and isinstance(results, list):
            logger.info(f"get_all_with_filters returned {len(results)} results.")
            return [row['id'] for row in results] # Assuming 'id' is the column name
        else:
            logger.warning("get_all_with_filters returned no results or results are not in expected format.")
            return []
